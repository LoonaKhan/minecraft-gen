...
class Colours:
    ...
    green = (0,255,0)
    light_green = (112,245,105)

    blue = (0,0,255)
    light_blue = (43, 174, 214)
...
def create_window(resolution, resizable=True, fullscreen=False, icon=None, caption=None):
    """
    Creates a window.
    ...
    window = pygame.display.set_mode((resolution[0], resolution[1]), is_resizable, is_fullscreen) # creates the window
...
def render_text(text, window, coords, colour=Colours.white, font = Fonts.default_32):
    message = font.render(f"{text}", True, colour)
    window.blit(message, (coords))

def show_text(text, window, coords, color=Colours.white, font = Fonts.default_32, duration=0.5):
